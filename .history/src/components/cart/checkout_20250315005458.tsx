...
const CheckoutModal = ({ isOpen, onClose, cartItems }:{isOpen:boolean,onClose:(data:boolean)=>void, cartItems:CartItem[]}) => {
  ...
  const handleInputChange = <T>(e:React.ChangeEvent<HTMLInputElement>, stateSetter:React.Dispatch<React.SetStateAction>) => {
  ...
  const validateCurrentStep = () => {
    ...
    if (currentStep === 1) {
      if (!selectedAddressId && showAddressForm) {
        for (const field of ['fullName', 'address', 'city', 'state', 'zipCode', 'phone']) {
                ...
                >
                  {selectedAddressId ? (
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-600">{shippingAddress.address}, {shippingAddress.city}</p>
                    </div>
                  ) : (
                ...
                {addressDropdownOpen && (
                  <div className="absolute left-0 right-0 mt-1 bg-white border rounded-md shadow-lg z-10">
                    {savedAddresses.map((addr) => (
                      <div 
                        key={addr.id}
                        className={`p-2 hover:bg-gray-50 cursor-pointer ${addr.id === selectedAddressId ? 'bg-red-50' : ''}`}
                        onClick={() => handleAddressSelect(addr.id)}
                      >
                        <p className="font-medium text-sm">
                          {addr.fullName} {addr.isDefault && <span className="text-xs text-red-800">(Default)</span>}
                        </p>
                        <p className="text-xs text-gray-600">{addr.address}, {addr.city}, {addr.state}</p>
                      </div>
                    ))}
                    <div 
                      ...
                      onClick={() => {
                        ...
                        setShippingAddress({fullName: '', address: '', city: '', state: '', zipCode: '', phone: ''});
                        setAddressDropdownOpen(false);
                      }}
                    >
                  ...
                  {['fullName', 'address', 'city', 'state', 'zipCode', 'phone'].map(field => (
                    <div key={field} className={field === 'fullName' || field === 'address' ? 'col-span-2' : ''}>
                      ...
                      <input 
                        ...
                        className={`w-full p-2 border rounded-md text-sm ${errors[field] ? 'border-red-500' : 'border-gray-300'}`}
            ...
            <div className="space-y-3">
              <div>
                ...
                <input 
                  ...
                  maxLength={19}
                  className={`w-full p-2 border rounded-md text-sm ${errors.cardNumber ? 'border-red-500' : 'border-gray-300'}`}
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full p-2 border rounded-md text-sm ${errors.cardHolder ? 'border-red-500' : 'border-gray-300'}`}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleInputChange(e, setPaymentMethod)}
                    className={`w-full p-2 border rounded-md text-sm ${errors.expiryDate ? 'border-red-500' : 'border-gray-300'}`}
                    placeholder="MM/YY"
                    maxLength={5}
                  />
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleInputChange(e, setPaymentMethod)}
                    className={`w-full p-2 border rounded-md text-sm ${errors.cvv ? 'border-red-500' : 'border-gray-300'}`}
                    placeholder="123"
                    maxLength={4}
                  />
