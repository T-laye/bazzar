...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, cartItems }) => {
  ...
  const [shippingAddress, setShippingAddress] = useState<IUserAddress>({
    city: '',
    state: '',
    zip_code: '',
    country:'',
  ...
  useEffect(() => {
    if (user && user.address && user.address.length > 0) {
      ...
      const defaultAddress = user.address.find(addr => addr.default_address) || user.address[0];
      if (defaultAddress) {
  ...
  // Handle changing selected address
  const handleAddressSelect = (addressId: string) => {
    const selected = savedAddresses.find(addr => addr?._id as string === addressId);
    if (selected) {
  ...
  const handleSubmitOrder = async () => {
    if (validateCurrentStep()) {
      ...
      try {
        ...
        // If this is a new address, you might want to save it to the user's document
        if (showAddressForm && !selectedAddressId) {
          // This would be an API call to save the new address
          // const savedAddress = await apiService.saveUserAddress(user.id, shippingAddress);
        }
                  ...
                  <div>
                    ...
                    {savedAddresses.length > 0 && (
                      <div className="mb-4 relative" ref={dropdownRef}>
                        <div 
                          className="flex justify-between items-center p-2 md:p-3 border rounded-md cursor-pointer hover:bg-gray-50"
