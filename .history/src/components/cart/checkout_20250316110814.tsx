...
import PaystackHook from './paystackPayment';
...
// console.log(sessio)
  ...
  const validateCurrentStep = () => {
    ...
    console.log('k',selectedAddressId,showAddressForm);
    if (currentStep === 1) {
  ...
  useEffect(() => {
    if (orderId) {
      console.log("Now proceeding with payment for Order ID:", orderId);
      buttonRef.current?.click(); // Clicks the button only when orderId is set
    }
  }, [orderId]); 
    ...
    if (validateCurrentStep()) {
      ...
      try {
        ...
        const payload:IOrder={
          ...
          billing:{
            name:`${session?.user.name?.first_name} ${session?.user.name?.last_name}`,
        ...
        createOrder(payload, {
          onSuccess: (data) => {
            ...
            // buttonRef.current?.click(); // Click the button only when the order is successfully created
            setCartItems([]);
        ...
        // isSuccess  && buttonRef.current?.click();
        // setCartItems([]);
        // setOrderComplete(true);
      } catch (error) {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return (
          <div>
            <h3 className="text-base md:text-lg font-medium mb-3 text-red-500">Shipping Address</h3>
                ...
                >
                  {selectedAddressId ? (
                    <div className="flex-1">
                      <p className="font-medium text-sm text-slate-800">{shippingAddress.country}</p>
      ...
      case 2:
        return (
          <div>
            <h3 className="text-base md:text-lg font-medium mb-3 text-red-500">Order Review</h3>
            
            {/* Shipping Details */}
            <div className="mb-3 p-2 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                <h4 className="font-medium text-sm text-slate-800">Shipping Address</h4>
            ...
            <div className="border-t pt-3">
              ...
              <div className="border-t pt-2 space-y-1">
                <div className="flex justify-between">
                  <span className="text-xs text-slate-800">Subtotal</span>
                  <span className="text-xs font-medium text-red-500">₦ {subtotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-slate-800">Shipping</span>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 transition-opacity flex items-center justify-center">
      <div ref={modalRef} className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg shadow-xl m-4">
        <div className="p-3 border-b flex justify-between items-center bg-red-800 text-white rounded-t-lg">
          <h2 className="text-lg font-bold">{orderComplete ? 'Order Completed' : 'Checkout'}</h2>
          <button onClick={() => onClose(false)} className="p-1 rounded-full hover:bg-red-700 transition-colors">
            <X size={20} />
          </button>
        </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between">
                {[
                  ...
                  { step: 2, icon: <Truck size={12} />, label: "Review" }
                ].map((item, index, arr) => (
        ...
        <PaystackHook amount={subtotal + totalPrice} email={session?.user?.email as string} buttonRef={buttonRef} orderId={orderId} showSuccessModal={setOrderComplete}/>
      </div>
    </div>
  );
};
