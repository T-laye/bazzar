...
export default function capability() {
  return (
    <CaliberationLayout title="Force guage Caliberations" >
      ...
      <p className="text-center mb-[10px] font-medium max-w-[200px] mx-auto text-[14px]">
      ...
      <ul className="list-disc text-[14px] font-medium mb-[10px] px-8">
       <li>Expect your force gauge to be calibrated and on its way back to you
       within 5 working days (or faster if you need expedited service.)</li> 
      </ul>
      <p className="text-[14px] mb-[10px] font-medium px-8">
      ...
      <ul className="px-8 list-disc mb-[10px]">
        {CaContent2.map((p, i) => {
          return (
            <li key={i} className="font-medium text-[14px]">
              {p.text}
            </li>
          );
        })}
      </ul>
      <p className="text-[14px] mb-[10px] font-medium">
        <span className="hover:underline text-primary font-medium">CertFlo</span> is our new custom, completely paperless calibration management
      ...
      <Link href="/" className="text-primary hover:underline text-[14px] font-medium">Click here to see a sample force gauge calibration certificate.</Link>
    </CaliberationLayout>
  );
}
